...
describe("Frontmatter JSON Schemas", () => {
  ...
  beforeAll(() => {
    ...
    commonSchema = JSON.parse(fs.readFileSync(path.join(schemasDir, "defs/common.schema.json"), "utf-8"));
    storySchema = JSON.parse(fs.readFileSync(path.join(schemasDir, "story.schema.json"), "utf-8"));
    epicSchema = JSON.parse(fs.readFileSync(path.join(schemasDir, "epic.schema.json"), "utf-8"));
  ...
  describe("common.schema.json", () => {
    ...
    it("should define storyType as string", () => {
      expect(commonSchema).toHaveProperty("$defs.storyType");
      expect((commonSchema as Record<string, Record<string, Record<string, string>>>).$defs.storyType.type).toBe("string");
    });
  ...
  describe("story.schema.json", () => {
    ...
    it("should accept story with any string type (types validated against config, not schema)", () => {
      const validate = ajv.compile(storySchema);
      const story = {
        id: "DS-001",
        title: "Custom Type",
  ...
  describe("task.schema.json", () => {
    ...
    beforeAll(() => {
      const schemasDir = path.join(__dirname, "../../../schemas");
      taskSchema = JSON.parse(fs.readFileSync(path.join(schemasDir, "task.schema.json"), "utf-8"));
    });
