...
describe("storyHoverProviderUtils", () => {
  ...
  describe("getTypeIcon", () => {
    const storyTypes: Record<string, StoryTypeConfig> = {
      feature: { template: "feature.template.md", description: "New functionality", icon: "lightbulb", emoji: "✨" },
      bug: { template: "bug.template.md", description: "Defect", icon: "bug", emoji: "🐛" },
      task: { template: "task.template.md", description: "Work item", icon: "tasklist", emoji: "📋" },
      chore: { template: "chore.template.md", description: "Maintenance", icon: "tools", emoji: "🔧" },
    };

    it("should return lightbulb for feature", () => {
      expect(getTypeIcon("feature", storyTypes)).toBe("✨");
    });

    it("should return bug icon for bug", () => {
      expect(getTypeIcon("bug", storyTypes)).toBe("🐛");
    });

    it("should return checklist for task", () => {
      expect(getTypeIcon("task", storyTypes)).toBe("📋");
    });

    it("should return wrench for chore", () => {
      expect(getTypeIcon("chore", storyTypes)).toBe("🔧");
    });

    it("should return folder for epic", () => {
      expect(getTypeIcon("epic", storyTypes)).toBe("📁");
    });

    it("should return default for unknown type", () => {
      expect(getTypeIcon("unknown" as StoryType, storyTypes)).toBe("📄");
    });
  });
  ...
  describe("formatHoverCard", () => {
    ...
    it("should format story hover card with all fields", () => {
      const result = formatHoverCard(mockStory, "story", undefined, storyTypes);
    ...
    it("should include sprint if present", () => {
      ...
      const result = formatHoverCard(storyWithSprint, "story", undefined, storyTypes);
    ...
    it("should capitalize type name", () => {
      ...
      const result = formatHoverCard(bugStory, "story", undefined, storyTypes);
    ...
    it("should include priority if non-default", () => {
      ...
      const result = formatHoverCard(priorityStory, "story", undefined, storyTypes);
  ...
  describe("isInFrontmatter", () => {
    it("should return true for line within frontmatter", () => {
      const lines = ["---", "id: DS-001", "epic: EPIC-001", "---", "# Content"];
    ...
    it("should return false for line after frontmatter", () => {
      const lines = ["---", "id: DS-001", "---", "# Content", "Body text"];
    ...
    it("should return true for unclosed frontmatter", () => {
      const lines = ["---", "id: DS-001", "epic: EPIC-001"];
  ...
  describe("getFieldDescription", () => {
    ...
    it("should return description for type field", () => {
      ...
      expect(result).toBe("Story type (validated against config.json storyTypes)");
    });
