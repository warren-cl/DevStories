...
export class StoryHoverProvider implements vscode.HoverProvider {
  ...
  provideHover(document: vscode.TextDocument, position: vscode.Position, _token: vscode.CancellationToken): vscode.Hover | null {
    ...
    if (inFrontmatter) {
      ...
      if (fieldMatch) {
        ...
        if (description) {
          ...
          const range = new vscode.Range(position.line, fieldMatch.start, position.line, fieldMatch.end);
    ...
    const cardContent = formatHoverCard(item, type, progress, this.configService?.config?.storyTypes);
