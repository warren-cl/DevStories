...
import { detectProjectName, generateConfigJson, generateSampleEpic, generateSampleStory, InitConfig } from "./initUtils";
...
export async function executeInit(options: InitOptions = {}): Promise<boolean> {
  ...
  // Always open config.json so the user can review/edit settings
  const doc = await vscode.workspace.openTextDocument(configUri);
  await vscode.window.showTextDocument(doc);

  // Show success message with next steps
  const action = await vscode.window.showInformationMessage("✅ DevStories initialized! Created .devstories/ directory.", "Create Epic");
